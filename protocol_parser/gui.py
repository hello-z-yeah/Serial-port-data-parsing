...
class ProtocolParserApp:
    def __init__(self, root: tk.Tk, monitor_port: str | None = None, monitor_baud: int = 9600):
        ...
        # 落盘缓冲由后台 _save_writer_loop 内部维护，主线程只往 _save_q 压栈
    ...
    def _adapt_serial_row_widths(self, event=None) -> None:
        """窗口 Configure 防抖：停止拖动 150ms 后再改控件宽度，避免拖拽期间 CPU 飙高。"""
        if event is not None and getattr(event, "widget", None) is not self.root:
            return
        job = getattr(self, "_adapt_serial_job", None)
        if job is not None:
            try:
                self.root.after_cancel(job)
            except Exception:
                pass
        self._adapt_serial_job = self.root.after(150, self._safe(self._adapt_serial_row_widths_apply))

    def _adapt_serial_row_widths_apply(self) -> None:
        """实际应用波特率列宽（仅在防抖结束后执行一次）。"""
        self._adapt_serial_job = None
        try:
    ...
    def _toggle_cmdlib_panel(self) -> None:
        if self.cmdlib_visible:
            ...
            try:
                ...
                pass
            # 开关只操作 pane，不读盘、不 destroy/重建行控件
    ...
    def _cmdlib_load(self, force: bool = False) -> None:
        """从磁盘读入指令库到内存。默认只加载一次；开关指令库不得再读盘。"""
    ...
    def _build_cmd_library_panel(self, parent: tk.Misc) -> None:
        ...
        ttk.Button(
            bar, text="新增", width=6, style="SendOutline.TButton",
        ...
        self._cmdlib_edit_row: str | None = None
        ...
        self.cmdlib_tree.bind("<Button-3>", self._safe(self._cmdlib_on_right_click))

        # 鼠标滚轮（仅在树区域内）
        def _on_mousewheel(event):
            if getattr(event, "delta", 0):
                self.cmdlib_tree.yview_scroll(int(-event.delta / 120), "units")
            elif getattr(event, "num", None) == 4:
                self.cmdlib_tree.yview_scroll(-1, "units")
            elif getattr(event, "num", None) == 5:
                self.cmdlib_tree.yview_scroll(1, "units")

        def _bind_wheel(_e=None):
            self.cmdlib_tree.bind_all("<MouseWheel>", _on_mousewheel)
            self.cmdlib_tree.bind_all("<Button-4>", _on_mousewheel)
            self.cmdlib_tree.bind_all("<Button-5>", _on_mousewheel)

        def _unbind_wheel(_e=None):
            self.cmdlib_tree.unbind_all("<MouseWheel>")
            self.cmdlib_tree.unbind_all("<Button-4>")
            self.cmdlib_tree.unbind_all("<Button-5>")

        self.cmdlib_tree.bind("<Enter>", _bind_wheel)
        self.cmdlib_tree.bind("<Leave>", _unbind_wheel)

        # 不再使用 Canvas / 40 行重控件；无 <Configure> bbox 全量计算
        self._cmdlib_row_widgets = None  # 兼容旧引用
        self._cmdlib_load()
        self._cmdlib_refresh_list()
        self._cmdlib_set_mode(self._cmdlib_mode.get() or "hex")

    def _cmdlib_on_right_click(self, event) -> None:
        """右键菜单：发送 / 清空 / 编辑。"""
        row = self.cmdlib_tree.identify_row(event.y)
        if row:
            self.cmdlib_tree.selection_set(row)
            self.cmdlib_tree.focus(row)
        menu = tk.Menu(self.cmdlib_tree, tearoff=0)
        menu.add_command(label="发送", command=self._safe(self._cmdlib_send_selected))
        menu.add_command(label="清空", command=self._safe(self._cmdlib_clear_selected))
        menu.add_command(label="编辑…", command=self._safe(self._cmdlib_edit_selected_dialog))
        try:
            menu.tk_popup(event.x_root, event.y_root)
        finally:
            menu.grab_release()

    def _cmdlib_selected_index(self) -> int | None:
        sel = self.cmdlib_tree.selection()
        if not sel:
            return None
        try:
            return int(sel[0])
    ...
    def _cmdlib_send_selected(self, _event=None) -> None:
        idx = self._cmdlib_selected_index()
        if idx is None:
            messagebox.showinfo("提示", "请先选中一条指令")
            return
    ...
    def _cmdlib_clear_selected(self, _event=None) -> None:
        idx = self._cmdlib_selected_index()
        if idx is None:
            return
        ...
        try:
            iid = str(idx)
            if self.cmdlib_tree.exists(iid):
                self.cmdlib_tree.selection_set(iid)
        except Exception:
            pass

    def _cmdlib_edit_selected_dialog(self) -> None:
        idx = self._cmdlib_selected_index()
        if idx is None:
            self._cmdlib_add_item()
            return
        items = list(self._cmdlib_current_list())
        if idx < len(items) and (items[idx].get("payload") or items[idx].get("name")):
            self._cmdlib_edit_item(items[idx])
        else:
            self._cmdlib_add_item()
    ...
    def _cmdlib_on_double_click(self, event) -> None:
        ...
        if not row or col not in ("#2", "#4"):
            return
        bbox = self.cmdlib_tree.bbox(row, col)
        if not bbox:
            return
        x, y, w, h = bbox
        col_key = "name" if col == "#2" else "payload"
        cur = self.cmdlib_tree.set(row, col_key)
        ...
        self._cmdlib_edit_entry = entry
        self._cmdlib_edit_row = row
        self._cmdlib_edit_col = col
        try:
            self._cmdlib_edit_index = int(row)
        except Exception:
            self._cmdlib_edit_index = None

        entry.bind("<Return>", lambda _e: self._cmdlib_commit_edit())
        entry.bind("<Escape>", lambda _e: (self._cmdlib_cancel_edit(), "break")[1])
        entry.bind("<FocusOut>", lambda _e: self._cmdlib_commit_edit())

    def _cmdlib_cancel_edit(self) -> None:
        ent = self._cmdlib_edit_entry
        self._cmdlib_edit_entry = None
        self._cmdlib_edit_row = None
        self._cmdlib_edit_col = None
        self._cmdlib_edit_index = None
        if ent is not None:
            try:
                ent.unbind("<FocusOut>")
    ...
    def _cmdlib_commit_edit(self) -> None:
        ent = self._cmdlib_edit_entry
        row = self._cmdlib_edit_row
        col = self._cmdlib_edit_col
        idx = self._cmdlib_edit_index
        # 先清空状态，避免 destroy 触发 FocusOut 再次进入
        self._cmdlib_edit_entry = None
        self._cmdlib_edit_row = None
        self._cmdlib_edit_col = None
        self._cmdlib_edit_index = None
        if ent is None or row is None or col is None or idx is None:
            if ent is not None:
                try:
                    ent.unbind("<FocusOut>")
                except Exception:
        ...
        old = items[idx] if idx < len(items) else {"id": "", "name": "", "payload": ""}
        ...
        if col == "#2":
            ...
            payload = new_val
        self._cmdlib_save_row(idx, payload, name)
        self._cmdlib_refresh_list()
        try:
            if self.cmdlib_tree.exists(str(idx)):
                self.cmdlib_tree.selection_set(str(idx))
                self.cmdlib_tree.focus(str(idx))
        except Exception:
            pass
    ...
    def _cmdlib_set_mode(self, mode: str) -> None:
        if mode not in ("hex", "ascii"):
            return
        if self._cmdlib_edit_entry is not None:
            self._cmdlib_cancel_edit()
        self._cmdlib_mode.set(mode)
        try:
            if mode == "hex":
                self.cmdlib_mode_btn.configure(text="HEX", style="Success.TButton")
            else:
                self.cmdlib_mode_btn.configure(text="ASCII", style="Danger.TButton")
        except Exception:
            pass
    ...
    def _cmdlib_refresh_list(self) -> None:
        """只更新 Treeview 行数据，绝不重建控件。"""
        tree = getattr(self, "cmdlib_tree", None)
        if tree is None:
            return
        if self._cmdlib_edit_entry is not None:
            self._cmdlib_cancel_edit()
        ...
        mode_label = "HEX" if self._cmdlib_mode.get() == "hex" else "ASCII"

        # 用 item 复用：固定 CMDLIB_MAX 槽位，iid = 下标字符串
        existing = set(tree.get_children(""))
        for i in range(self.CMDLIB_MAX):
            if i < len(saved):
                name = saved[i].get("name") or ""
    ...
    def _cmdlib_open_cycle_config(self) -> None:
        """配置循环发送：勾选 + 间隔 + 手动排序（上移/下移/置顶/置底）。

        列表从上到下的顺序 = 循环发送顺序。
        打开时：已在循环序列中的指令按原顺序排在前面，其余库内指令跟在后面。
        """
        ...
        pool = {it.get("id"): it for it in items}

        # 显示顺序：先已配置循环的（按 seq 顺序），再未入循环的库指令
        ...
        ttk.Label(
            top,
            text="勾选参与循环的指令，双击「间隔」可改 ms；列表顺序 = 发送顺序（可用右侧按钮手动排序）",
        ...
        def _move_edge(to_top: bool) -> None:
            iid = _selected_iid()
            if not iid:
                return
            children = list(tree.get_children(""))
            if not children or iid not in children:
                return
            # Tk Treeview.move 的 index 必须是整数；"end" 在部分版本会 TclError
            tree.move(iid, "", 0 if to_top else max(0, len(children) - 1))
            tree.selection_set(iid)
            tree.focus(iid)
            tree.see(iid)

        ttk.Button(side, text="上移 ↑", width=10, command=lambda: _move(-1)).pack(pady=(0, 4), fill="x")
        ttk.Button(side, text="下移 ↓", width=10, command=lambda: _move(1)).pack(pady=(0, 4), fill="x")
        ttk.Button(side, text="置顶", width=10, command=lambda: _move_edge(True)).pack(pady=(0, 4), fill="x")
        ttk.Button(side, text="置底", width=10, command=lambda: _move_edge(False)).pack(pady=(0, 12), fill="x")

        def _toggle_all(on: bool) -> None:
            mark = "☑" if on else "☐"
        ...
        ttk.Button(side, text="全选", width=10, command=lambda: _toggle_all(True)).pack(pady=(0, 4), fill="x")
        ttk.Button(side, text="全不选", width=10, command=lambda: _toggle_all(False)).pack(pady=(0, 4), fill="x")

        # 填充行（iid 用指令 id，保证排序/保存稳定）
        ...
        # 单击「参与」列切换勾选；双击「间隔」列就地编辑
        edit_entry: dict = {"w": None, "cancelled": False}

        def _cancel_edit() -> None:
            w = edit_entry.get("w")
            edit_entry["w"] = None
            edit_entry["cancelled"] = True
            if w is not None:
                try:
                    w.unbind("<FocusOut>")
                except Exception:
        ...
        def _commit_delay(row: str, ent: tk.Entry) -> None:
            if edit_entry.get("cancelled"):
                edit_entry["cancelled"] = False
                return
            if edit_entry.get("w") is not ent:
                return
            try:
                raw = ent.get().strip()
                d = max(10, int(raw))
        ...
        def _on_dbl(event) -> None:
            ...
            if not bbox:
                return
            x, y, w, h = bbox
            cur = tree.set(row, "delay")
            ent = tk.Entry(tree, font=("Microsoft YaHei UI", 10), justify="center")
            ent.place(x=x, y=y, width=max(w, 50), height=h)
            ent.insert(0, cur)
            ent.select_range(0, "end")
            ent.focus_set()
            edit_entry["w"] = ent
            edit_entry["cancelled"] = False
            ent.bind("<Return>", lambda _e: _commit_delay(row, ent))
            ent.bind("<FocusOut>", lambda _e: _commit_delay(row, ent))
            ent.bind("<Escape>", lambda _e: (_cancel_edit(), "break")[1])

        tree.bind("<Button-1>", _on_click, add="+")
        tree.bind("<Double-1>", _on_dbl, add="+")

        # 键盘快捷：Alt+↑ / Alt+↓ 排序
        top.bind("<Alt-Up>", lambda _e: (_move(-1), "break")[1])
        top.bind("<Alt-Down>", lambda _e: (_move(1), "break")[1])
    ...
    def _load_protocols(self) -> None:
        ...
        # 内置协议走内存缓存，避免每次扫描 product/ 时重复读盘
    ...
    # 原始数据落盘已统一走 _save_q + 后台 _save_writer_loop，
    # 不再使用主线程 _save_raw_buf / _flush_save_raw_buf。
    ...
    def _process_ui_queue(self) -> None:
        ...
        try:
            ...
            # 固定 ~50ms 批量拉队列，避免串口风暴打满主线程
            delay = 50 if self._ui_queue else 80
