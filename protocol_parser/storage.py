...
class RawDataWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self._failed = False
        self._last_error = ""
        self._error_signal_triggered = False
    ...
    def _fail(self, exc: BaseException) -> None:
        ...
        with self._state_lock:
            ...
            first_error = not self._error_signal_triggered
            if first_error:
                self._error_signal_triggered = True
        self._accepting = False
        self._stop_requested.set()
        # 致命错误只向主线程抛一次 on_error，避免高频 I/O 失败时
        # 成百上千个弹窗/回调把 GUI 事件循环卡死；后续失败仅累加指标。
        if first_error and self.on_error is not None:
            try:
